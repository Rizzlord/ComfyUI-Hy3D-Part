    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if config.get("conditioner", None) is not None:
            ...
            conditioner = vae
        scheduler = instantiate_from_config(config["scheduler"])
        # Fix checkpoint path to use the correct filename
        p3sam_checkpoint_options = [
            os.path.join(ckpt_path, "p3sam.pt"),    # Correct filename
            os.path.join(ckpt_path, "p3sam.ckpt"),  # Fallback
        ]
        
        p3sam_ckpt_path = None
        for ckpt_option in p3sam_checkpoint_options:
            if os.path.exists(ckpt_option):
                p3sam_ckpt_path = ckpt_option
                break
        
        if p3sam_ckpt_path is None:
            # If no checkpoint found locally, use the original path and let it download
            p3sam_ckpt_path = os.path.join(ckpt_path, "p3sam.pt")
